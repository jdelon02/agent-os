...
<step number="8.5" name="enhanced_mcp_learning_integration">

### Step 8.5: Enhanced MCP Learning Integration

<step_metadata>
  <action>apply enhanced MCP learning for task execution effectiveness</action>
  <purpose>capture implementation decisions and error patterns for cross-project learning</purpose>
  <memory_integration>dual memory storage with session and cross-project learning</memory_integration>
</step_metadata>

<enhanced_learning_integration>
  <!-- Apply enhanced MCP learning integration for task execution -->
  <include>@reference-docs/instructions/support-workflows/enhanced-mcp-learning-integration.md</include>
  PARAMETERS:
  ...
  <execution_specific_learning>
    # Apply vibe-check for implementation approach validation
    CALL: vibe-check-vibe_check
    PARAMETERS:
      - plan: "Implementation approach: {implementation_summary}"
      - userRequest: "Execute task: {task_description}"
      - phase: "implementation"
      - focusAreas: ["implementation_assumptions", "approach_validation"]
      - sessionId: "{PROJECT_ENTITY_NAME}-execute-{timestamp}"
    
    # Store implementation decision patterns
    CALL: mcp-memory-keeper-context_save
    PARAMETERS:
      - key: "implementation-decision-{PROJECT_ENTITY_NAME}-execute-{timestamp}"
      - value: "Task: {task_description}, Approach: {implementation_approach}, Validation: {vibe_check_results}"
    ...
    IF implementation_complexity == "high":
      ...
      PARAMETERS:
        - thought: "Complex implementation decision for {task_description} - analyzing approach systematically"
        - thoughtNumber: 1
        - totalThoughts: 3
        - nextThoughtNeeded: true
        - sessionId: "{PROJECT_ENTITY_NAME}-execute-{timestamp}"
      
      # Store thinking patterns for complex implementations
      CALL: mcp-memory-keeper-context_save
      PARAMETERS:
        - key: "implementation-thinking-{PROJECT_ENTITY_NAME}-execute-{timestamp}"
        - value: "Task: {task_description}, Complexity: {implementation_complexity}, Thinking: {thought_sequence}"
        - category: "thinking_patterns"
        - priority: "high"
    
    # Create implementation pattern entities in Memento
    CALL: memento-mcp-create_entities
    PARAMETERS:
      - entities: [{
          "name": "{PROJECT_ENTITY_NAME}-implementation-pattern-{timestamp}",
          "entityType": "implementation_pattern",
          "observations": [
            "Task: {task_description}",
            "Implementation Approach: {implementation_approach}",
            "Complexity Level: {implementation_complexity}",
            "Validation Results: {vibe_check_summary}",
            "Sequential Thinking Applied: {thinking_applied}",
            "Execution Context: Task implementation phase"
          ]
        }]
    ...
    PARAMETERS:
      - relations: [{
          "from": "{PROJECT_ENTITY_NAME}-implementation-pattern-{timestamp}",
          "to": "{PROJECT_ENTITY_NAME}",
          "relationType": "implementation_pattern_of",
          "metadata": {
            "task_type": "{task_type}",
...
<learning_benefits>
  <implementation_quality>
    - Validates implementation assumptions before development starts
    - Applies structured thinking to complex implementation decisions
    - Captures effective implementation approaches for reuse
    - Builds cross-project knowledge of successful patterns
  </implementation_quality>
  
  <error_prevention>
    - Identifies potential implementation issues early
    - Leverages previous error patterns to avoid similar mistakes
    - Applies enhanced error resolution when implementation blocks occur
    - Stores error patterns for organizational learning
  </error_prevention>
  
  <knowledge_building>
    - Creates searchable implementation decision database
    - Enables pattern recognition across similar tasks
    - Supports junior developers with proven approaches
    - Builds organizational implementation intelligence
  </knowledge_building>
</learning_benefits>

<instructions>
  ACTION: Apply enhanced MCP learning integration for task execution
  VALIDATE: Implementation approaches using vibe-check before proceeding
  ANALYZE: Complex decisions using sequential-thinking when needed
  STORE: Implementation patterns and decisions for cross-project learning
  LEVERAGE: Previous implementation patterns and error learnings
</instructions>
...
<memory_enhanced_progress_tracking>
  ...
  FOR_EACH: blocked_task
    ...
    # Apply enhanced error resolution with MCP tools
    <enhanced_error_resolution>
      # Apply error-resolution-via-memory.md with MCP enhancements
      ...
      # Apply vibe-learn for implementation blocking patterns
      CALL: vibe-check-vibe_learn
      PARAMETERS:
        - mistake: "Task blocked: {task_description} - {blocking_issue}"
    ...
    PARAMETERS:
      ...
      - value: "{task_description}: {blocking_issue} - Status: Blocked after {attempt_count} attempts - Enhanced error resolution applied"
      - category: "error"
      - priority: "high"
    ...
    PARAMETERS:
      - entities: [{
          "name": "{PROJECT_NAME}-enhanced-blocking-{task_type}-{timestamp}",
          "entityType": "enhanced_implementation_blocker",
          "observations": [
            "Task: {task_description}",
            "Blocker: {blocking_issue}",
            "Attempts: {attempt_count}",
            "Tech Stack: {PRIMARY_TECH}",
            "Enhanced Error Resolution Applied: {error_resolution_summary}",
            "Vibe Learn Pattern Stored: {vibe_learn_category}",
